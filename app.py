...
# 2. Kompatibilitäts-Fix für DepthwiseConv2D
# Wir erstellen eine eigene Klasse, die das störende 'groups' Argument ignoriert
class FixedDepthwiseConv2D(tf.keras.layers.DepthwiseConv2D):
    def __init__(self, **kwargs):
        if 'groups' in kwargs:
            kwargs.pop('groups')  # Entfernt das problematische Argument
...
def load_resources():
    ...
    try:
        # Hier sagen wir Keras, dass es unsere 'Fixed' Klasse statt der normalen verwenden soll
        custom_objects = {
            "DepthwiseConv2D": FixedDepthwiseConv2D
        }
        model = load_model(model_path, compile=False, custom_objects=custom_objects)
    except Exception as e:
        st.error(f"Kritischer Fehler beim Laden des Modells: {e}")
        st.stop()
